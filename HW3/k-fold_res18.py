...
class myResnet18(nn.Module):
    def __init__(self) -> None:
        ...
        self.resnet18.fc = nn.Linear(num_ftrs, 11)
...
# In[11]:
lr_lambda = lambda epoch: (2 ** epoch) * 1e-3 if epoch <= 10 else (0.5 ** (epoch // 10 - 1))
...
for fold, (train_set_id, valid_set_id) in enumerate(k_fold.split(files)):
    ...
    # scheduler1 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 5, 1e-5)
...
res18_0.load_state_dict(torch.load(f"{_exp_name}_fold_0.ckpt"))
res18_0.eval()
...
res18_1.load_state_dict(torch.load(f"{_exp_name}_fold_1.ckpt"))
res18_1.eval()
...
res18_2.load_state_dict(torch.load(f"{_exp_name}_fold_2.ckpt"))
res18_2.eval()
...
res18_3.load_state_dict(torch.load(f"{_exp_name}_fold_3.ckpt"))
res18_3.eval()
...
res18_4.load_state_dict(torch.load(f"{_exp_name}_fold_4.ckpt"))
res18_4.eval()
...
df.to_csv("submission_res18.csv",index = False)
